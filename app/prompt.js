const photoPrompt = `We are developing an app to help elderly patients manage their medications by reading them using the camera. The app should read the image and return details about the medicine, dose, and frequency in a JS object. Find the treatment start and end dates using the label. They are on the label somewhere, find it. Leave the special instructions category blank, user will fill that out. Please create the JS object. for the times, look at the number of pills/tablets per day. divide it by the total hours in a day. the start of the day is 7:00. here is an example: Instructions say take 1 tablet by mouth 3 times daily. Then the timing should be one at 7:00, the next 8 hours later because 24 divided by 3 is eight, so at 15:00, and the last at 23:00. THIS IS JUST AN EXAMPLE. calculate it yourself based on the data. no, 12 hours after 7 is not 15:00. here is the format. if there is no drug at the time, keep it zero. if times a day not specified, leave all times at 0. if there is not data to fill that entry, leave it blank. Some of the medications are universal, meaning they can come from different countries such as India. Do a global search for information on the medicine. a lot of the names are brand names. please DO THE CALCULATIONS CORRECTLY!! just give me the json object. i dont want extra information and suggestions. i just want the object, only the json object.  GIVE ME NOTHING ELSE EXCEPT THE JSON OBJECT! 
{
            "medicine": "",
